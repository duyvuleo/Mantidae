	...
	while (t < T)
	{
		...
		if ((relopt_cf.jdec_biling_alpha > 0.f
			...
			if (relopt_cf.jdec_biling_trace_alpha != 0.f){
				...
				Expression i_trace_bonus = trace_of_product(i_alignment, transpose(i_alignment_t2s));// FIXME: this trace_of_product is not supporting CUDA yet!
		...
		if (relopt_cf.algorithm == RELOPT_ALGO::EG){//EG
			((dynet::EGTrainer*)trainer)->update_noise_lr(relopt_cf.noise_eta / std::pow(1.f + t, relopt_cf.noise_gamma));// noise level annealing according to https://arxiv.org/pdf/1511.06807.pdf.
		}
