/*
 * Joint training framework for NMT
 *
...
struct AttentionalModel {
	...
	//---------------------------------------------------------------------------------------------
	// For joint training framework
	Expression BuildGraphWithPreds_Batch(const std::vector<std::vector<int>> &sources, const std::vector<std::vector<int>>& targets
		, ComputationGraph& cg, ModelStats& tstats
		, std::vector<Expression>& v_dist_preds);
	Expression BuildGraphWithPreds_Batch(const std::vector<std::vector<int>> &sources, std::vector<Expression>& v_dist_preds/*distributional sources*/
		, ComputationGraph& cg, bool reverse=true);
	Expression BuildGraphWithPreds_Batch(std::vector<Expression>& v_dist_preds/*distributional sources*/, const std::vector<std::vector<int>> &targets
		, ComputationGraph& cg);
	void StartNewInstance_Batch(unsigned batch_size
		, std::vector<Expression>& v_dist_preds/*distributional sources*/
		, ComputationGraph &cg);
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildGraphWithPreds_Batch(const std::vector<std::vector<int>> &sources, const std::vector<std::vector<int>>& targets
	, ComputationGraph& cg, ModelStats& tstats
	, std::vector<Expression>& v_dist_preds/*batched distributional prediction vectors*/) 
{
	v_dist_preds.clear();
	...
	for (unsigned t = 0; t < tlen; ++t) {
		...
		Expression i_r_t = AddInput_Batch(words, t, cg);
		Expression i_softmax_pred = softmax(i_r_t);
		if (t != tlen - 1) v_dist_preds.push_back(i_softmax_pred);// exclusing EOS prediction
		Expression i_err = pick(-log(i_softmax_pred), next_words);
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildGraphWithPreds_Batch(const std::vector<std::vector<int>> &sources, std::vector<Expression>& v_dist_preds
	, ComputationGraph& cg, bool reverse) 
{
	ModelStats tstats;// unused for now
	StartNewInstance_Batch(sources, cg, tstats);// batched vector representation of sources
	ComputeTrgWordEmbeddingMatrix(cg);

	std::vector<Expression> v_errs;// error collection

	int tlen = v_dist_preds.size();// desired target length (exclusing BOS and EOS predictions)
	unsigned batch_size = sources.size();

	// collect batched expected word embeddings
	std::vector<Expression> i_wes(tlen + 1);
	i_wes[0] = concatenate_to_batch(std::vector<Expression>(batch_size, lookup(cg, p_ct, kTGT_SOS)));// known BOS embedding (batched)
	for(auto t : boost::irange(0, tlen)){
		auto ct = t;
		if (reverse == true) ct = tlen - t - 1;
		
		i_wes[t + 1] = GetWordEmbeddingVector(v_dist_preds[ct]);
	}

	// relaxed generation step
	std::vector<unsigned> v_EOSs(batch_size, kTGT_EOS);
	for(auto t : boost::irange(0, tlen + 1)) {
		...
		if (t >= tlen){// for predicting EOS
			i_err = pickneglogsoftmax(i_r_t, v_EOSs);
		}
		else{// for predicting others
			Expression i_logsoftmax = log_softmax(i_r_t);

			auto ct = t;
			if (reverse == true) ct = tlen - t - 1;
		
			i_err = -transpose(v_dist_preds[ct]) * i_logsoftmax;
		}
		
		v_errs.push_back(i_err);
	}

	Expression i_nerr = sum_batches(sum(v_errs));
	return i_nerr;
}

template <class Builder>
void AttentionalModel<Builder>::StartNewInstance_Batch(unsigned batch_size
	, std::vector<Expression>& v_dist_preds
	, ComputationGraph &cg)
{
	int SOS = 0, EOS = 1;
	std::vector<Expression> exp_wrd_embeddings;
	exp_wrd_embeddings.push_back(concatenate_to_batch(std::vector<Expression>(batch_size, lookup(cg, p_cs, SOS/*hacky*/))));// BOS
	for(auto t : boost::irange(0, (int)v_dist_preds.size())){
		exp_wrd_embeddings.push_back(GetWordEmbeddingVector(v_dist_preds[t]));
	}
	exp_wrd_embeddings.push_back(concatenate_to_batch(std::vector<Expression>(batch_size, lookup(cg, p_cs, EOS/*hacky*/))));// EOS

	slen = exp_wrd_embeddings.size();
	std::vector<Expression> source_embeddings;
	if (!rnn_src_embeddings) {
		for (unsigned s = 0; s < slen; ++s) 
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildGraphWithPreds_Batch(std::vector<Expression>& v_dist_preds/*distributional sources*/, const std::vector<std::vector<int>> &targets
	, ComputationGraph& cg) 
{
	ComputeSrcWordEmbeddingMatrix(cg);
	StartNewInstance_Batch(targets.size(), v_dist_preds, cg);	
	...
	for (unsigned t = 0; t < tlen; ++t) {
		for(size_t bs = 0; bs < targets.size(); bs++){
			words[bs] = (targets[bs].size() > t) ? (unsigned)targets[bs][t] : 1/*EOS, hacky*/;
			next_words[bs] = (targets[bs].size() > (t+1)) ? (unsigned)targets[bs][t+1] : 1/*EOS, hacky*/;
		}
